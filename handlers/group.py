...
@events.register(admin_cmd(pattern="(ban|unban) ?(.*)", allow_sudo=True))
async def _(event):
    ...
    if reply_msg_id:
        ...
        return False
    try:
        await events(EditBannedRequest(event.chat_id, to_ban_id, rights))
...
@events.register(admin_cmd(pattern="pgs ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    if event.reply_to_msg_id:
        ...
        if input_str:
            from_user = await events.get_entity(input_str)
            logger.info(from_user)
        async for message in events.iter_messages(
            event.chat_id,
            ...
            msgs.append(message)
            if len(msgs) == 100:
                await events.delete_messages(event.chat_id, msgs, revoke=True)
                msgs = []
        if len(msgs) <= 100:
            await events.delete_messages(event.chat_id, msgs, revoke=True)
            msgs = []
...
@events.register(admin_cmd(pattern="(ban|unban) ?(.*)"))
async def _(event):
    # Space weirdness in regex required because argument is optional and other
    # commands start with ".unban"
    if event.fwd_from:
...
@events.register(admin_cmd(pattern="affk(?: |$)(.*)", allow_sudo=True))
@errors_handler
async def promote(promt):
    """ For .promote command, promotes the replied/tagged person """
    # Get targeted chat
